...
real*8, external :: T_ions, T_electrons, f_temperature
...
psi_n_start = 0.10d0          ! limit domain to psi_start:psi_end (in normalised psi)
psi_n_end   = 0.66d0 
...
if (restart_particles) then
  ...
  partreader = event(read_action(filename='steady_particles_gc.h5'))
...
!                                     uniform_space_rej_vars=[-2,1], charge = +1, T_particles=T_ions)
                                     uniform_space_rej_vars=[-2,1], charge = +1, T_particles=f_temperature)
  call cpu_time(t1)
...
!                      do_dirichlet_open_n0 = .false.,   do_dirichlet_corners_n0 = .false., do_neumann_n0 = .true.,      &
                      do_dirichlet =.false.,                                                                             &
                      fractional_digits = 9, calc_integrals=.false., to_vtk=.true., to_h5=.false., basename='profiles', nsub=5)
...
!                      do_dirichlet_open_n0 = .false.,   do_dirichlet_corners_n0 = .false., do_neumann_n0 = .true. ,     &
                      do_dirichlet =.false.,                                                                             &
                      fractional_digits = 9, calc_integrals=.true., to_vtk=.false., to_h5=.false., basename='density', nsub=5)
...
function f_temperature(psi) result(f)
  use domain
  use equil_info
  use mod_normalisations
  use phys_module, only : T_scale_factor
  real*8,  intent(in) :: psi
  real*8 :: Z, Z_xpoint
  ...
  call temperature(xpoint, xcase, Z, Z_xpoint, psi, ES%psi_axis, ES%psi_bnd,&
                   zT,dT_dpsi,dT_dz,dT_dpsi2,dT_dz2,dT_dpsi_dz,dT_dpsi3,dT_dpsi_dz2, dT_dpsi2_dz)

  f = zT * Tev_norm * T_scale_factor
